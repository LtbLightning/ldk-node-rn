...
export class Config extends NativeLoader {
    ...
    async create(storageDirPath, esploraServerUrl, network, listeningAddress, defaultCltvExpiryDelta) {
        this.id = await this._ldk.createConfig(storageDirPath, esploraServerUrl, network, listeningAddress == null ? null : `${listeningAddress.ip}:${listeningAddress.port}`, defaultCltvExpiryDelta);
        return this;
    }
}
