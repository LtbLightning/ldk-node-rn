import { Address, PaymentDetails, PaymentHash, PaymentPreimage, PaymentSecret, PublicKey, Txid, } from './Bindings';
...
export class Node extends NativeLoader {
    ...
    async connectOpenChannel(nodeId, address, channelAmountSats, pushToCounterpartyMsat, channelConfig = null, announceChannel) {
        return await this._ldk.connectOpenChannel(this.id, nodeId, address, channelAmountSats, pushToCounterpartyMsat, channelConfig, announceChannel);
    }
    /**
    ...
    async payment(paymetHash) {
        ...
        return new PaymentDetails(new PaymentHash(paymentDetails.hash), new PaymentPreimage(paymentDetails.preimage), new PaymentSecret(paymentDetails.secret), paymentDetails.amountMsat, getPaymentDirection(paymentDetails.direction), getPaymentStatus(paymentDetails.status));
    }
    /**
