...
export class Builder extends NativeLoader {
     ...
     * @returns {Promise<boolean>}
     */
    async setListeningAddresses(listeningAddresses) {
        return await this._ldk.setListeningAddresses(this.id, listeningAddresses.map((listeningAddress) => addressToString(listeningAddress)));
    }
    /**
     * Create node
